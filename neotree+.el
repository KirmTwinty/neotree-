...
(defun neop-delete ()
  "Delete selection."
  (interactive)
  (if (not (null neop-list-deletion))
      (if (> (length neop-list-deletion) 1)
	  (if (yes-or-no-p (format "Do you really want to delete [%d] files?" (length neop-list-deletion)))
	      (progn
		...
		(setq neop-list-deletion nil)
		(neop-update-neotree)))
	(if (yes-or-no-p (format "Do you really want to delete %s?" (car neop-list-deletion)))
	    (progn
